...
class ControllerCategory():
    ...
    def updateAllCategories(self):
        ...
        for id in allCategories:
                ...
                if products == False:
                    products = []
                parent_id=allCategories[id]["parent_id"]
                complete_name=allCategories[id]["complete_name"]
                newCategory = Category(id,name,description,products,parent_id,complete_name)
    ...
    def showCategories(self):
        categories=""
        for id in self.__category:
            category=self.__category[id]
            categories+=str(id) + " - "+ str(category.getComplete_Name()) + "\n"
        categories+="0 - Productos sin carta"
        return categories
